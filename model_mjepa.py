...
class Encoder(nn.Module):
    ...
    def forward(self, x, masks=None): # [bs, length] 1 4094
        x = self.embed(x)    # [bs, length, dim] 1 4094 512

        pos_emb = self.pos_emb(x) # [bs, length, dim] 1 4094 512
        # Apply masks if provided
        if masks is not None:
            x = apply_masks(x, masks)  # Updated to align with your provided masking function
        for block in self.blocks:
            x = block(x)  # [bs, unmasklength, dim] 1 3966 512
        x = self.norm(x)
        return x
...
class Predictor(nn.Module):
    ...
    def forward(self, x, original, masks_x, masks):
        ...
        x = self.predictor_embed(x) # [bs, unmasklength, dim] 1 3966 256

        positions= self.predictor_pos_embed(original,True) # [bs, unmasklength, dim] 1 3966 256
        x_pos_embed = positions.repeat(B, 1, 1)
        x += apply_masks(x_pos_embed, masks_x)
        _, N_ctxt, D = x.shape

        pos_embs = apply_masks(x_pos_embed, masks)     # mask posiiton info

        pred_tokens = self.mask_token.repeat(pos_embs.size(0), pos_embs.size(1), 1)
        pred_tokens += pos_embs  # [bs, masklength, dim] 1 64 256
        x = torch.cat([x, pred_tokens], dim=1)   #  torch.Size([1, 4030, 256])
