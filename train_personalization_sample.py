...
def parse_args():
    ...
    parser.add_argument('--dislike_path', type=str, default="./preference/dislike", help='dislike path')
    parser.add_argument('--like_path', type=str, default="./preference/like", help='dislike path')
    ...
    parser.add_argument('--alpha', type=float, default=0.1, help='Margin.')
    ...
    parser.add_argument('--load_checkpoint', type=str, default='./preference/jepa-latest.pth.tar', help='Path to the checkpoint to load.')
...
def main():
    ...
    # Training loop
    dislike,masks_pred,masks_enc=np.load(args.dislike_path, allow_pickle=True)
    like,masks_pred_new,masks_enc_new = np.load(args.like_path, allow_pickle=True)
    dislike=dislike.to(args.device)
    masks_pred=masks_pred.to(args.device)
    masks_enc=masks_enc.to(args.device)
    like=like.to(args.device)
    masks_pred_new=masks_pred_new.to(args.device)
    masks_enc_new=masks_enc_new.to(args.device)
    ...
    for epoch in range(args.epochs):
        ...
        def train_step():
            ...
            def forward_context(data,masks_pred,masks_enc):
            ...
            with torch.amp.autocast('cuda',dtype=torch.bfloat16, enabled=args.use_bfloat16):
                h = forward_target(like,masks_pred_new)  # positive
                z = forward_context(like,masks_pred_new,masks_enc_new) #prediction
                l1_loss = loss_fn(z, h)
                h2 = forward_target(dislike,masks_pred)  # negative
                neg=F.pairwise_distance(z,h2)
                pos=F.pairwise_distance(z,h)
